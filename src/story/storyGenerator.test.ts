...
import {
  createStoryProgress, createStoryVocabularyMission, isCorrectSequence, pickStory
} from './storyGenerator';
...
describe('이야기 선택과 활동 생성', () => {
  ...
  it('선택지와 순서 활동을 섞고 진행 상태를 만든다', () => {
    ...
    const sequenceState = progress.activities.find((activity) => activity.activityId.includes('sequence'))!;
    const sequence = story.activities.find((activity) => activity.id === sequenceState.activityId)!;
