...
- 모양블록: 7개 칠교 조각으로 만드는 3단계 30문제와 회전 가능한 블록을 놓는 8×8 줄 채우기, 날짜별 오늘의 모양, 별·배지·최고 기록과 이어하기
- 모양블록은 맞는 배치·잘못된 배치·줄 제거를 구분하는 짧은 효과음과 지원 기기의 선택 가능한 손끝 반응을 제공하며 동작 줄이기 설정을 따른다.
- 작은 화면에서는 칠교 조각을 큰 가로 목록으로 보여 주고 다음 조각을 자동으로 가운데에 맞춘다. 조작 버튼과 블록 트레이는 화면 아래에서 판과 함께 보이며, 터치 드래그는 손가락 위쪽의 목표도 인식한다.
- 칠교 난이도는 첫걸음의 조각선 도움을 세 구간으로 줄이고, 쑥쑥은 필요한 조작을 4회에서 13회, 척척은 뒤집기를 포함해 20회에서 29회까지 한 문제씩 완만하게 높인다.
- 오늘의 추천은 이야기·저울·숫자 길·모양블록 중 아직 끝내지 않은 놀이를 우선하며, 모양블록 기록은 성장 숲의 공간 감각 나무와 보호자 요약에 반영
...
├─ shape-block/ 칠교 그림 완성과 8×8 줄 채우기
